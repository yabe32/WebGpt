...
Die erste Einrichtung erzeugt ein Admin-Konto. Weitere Konten können ausschließlich über das geschützte Admin-Panel erzeugt werden; es gibt keine öffentliche Registrierung. Chats, Sessions und Artefakte tragen eine Konto-ID und werden bei jedem Endpunkt geprüft. Das Admin-Panel zeigt nur Nutzungsmetadaten, nicht die Inhalte fremder Unterhaltungen. App-Stundenlimits sowie optionale Tokenlimits für fünf Stunden und eine Woche werden vor jedem Modellturn gezählt und durchgesetzt; `0` bedeutet unbegrenzt. Die Rate-Limits des einen verbundenen ChatGPT-Kontos bleiben global. Weil Codex Tokenwerte asynchron meldet, kann der aktuell laufende Turn sein Tokenlimit noch überschreiten; der nächste Turn wird zuverlässig blockiert.
